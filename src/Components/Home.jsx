...
i++;
   ...
   async function getBagdata(){
       const data=await fetch(`https://fagbag.onrender.com/products?page=1&pagesize=5`).then(d=>d.json());
       setBag(data);
       
   }
   async function getpridata(){
    const data=await fetch(`https://fagbag.onrender.com/previous?page=1&pagesize=5`).then(d=>d.json());
    setPrebag(data);
}
